...
evabyte* unpack(evabyte* in,evabyte *out,uchar* key){
    ...
    if(qqdecrypt((in->data+14),((in->len)-15),key,outstr,&outlen)!=1)
       {
        printf("\n%X\n",*(in->data+4));
        printf("解密错误");
        return NULL;
...
LONDATA* eva_login_init(){
    ...
    memset(logindata,0,sizeof(LONDATA));
...
int eva_login_verify_send(EVA *eva,LONDATA* logindata){
    ...
    uchar gdfix[18]={0x00,0x10,0x85,0x6D,     0x43,0x47,0x0F,0xA7,
                     0xFC,0x45,0xAD,0x26,     0x12,0x6A,0x59,0x87,
                     0x6A,0x27};  //the first and two bytes is len
    putevabyte(&verifybyte,gdfix,18); //71-90
    ...
    uchar gdddfix[4]={0x01, 0x19, 0x00 ,0x01, };
    ...
    decryptbyte.len+=2; *(decryptbyte.data+decryptbyte.len-1)=outlen;   //79-80
    //printf("\n%d\n",*(decryptbyte.data+decryptbyte.len));
    ...
    uchar noknowdata[87]={   0x00,0x14, //二十位数据key+crc32 前面为key后面crc32
			     ...
			     0xA6,0x44,0xBB,0x9D,   0xF0,0x6B,0xBF,0xAB,
			     0x0E,0xD6,0x05,0x7A,   0xCC,0x34,0x95,0x9E,
                              //根据QQ版本而固定的[QQ_LOGIN_FIX_DD_KEY1 后面要用
    ...
    uchar noknowfix[43]={0x23,0x1A,0x00,0x10,	0xE4,0xF4,0x64,0xF4,
                         0xD7,0x2C,0x9E,0xF1,	0x85,0xFD,0x01,0x3B,
                         0x64,0x61,0x15,0xDA,	0x02,0x20,0x82,0x45,
                         0x7D,0x00,0x10,0x68,	0x4B,0x2D,0xBE,0x4B,
                         0xE4,0x8E,0x1E,0xE4,	0x8B,0xA1,0xC8,0x74,
                         0x37,0x15,0x5A};
    putevabyte(&decryptbyte,noknowfix,43);
    decryptbyte.len+=269; //the last of 269 zero
    int sendlen=eva_net_send(eva->net,pack(&decryptbyte,&sendbyte,cmd,key,eva));
    if(sendlen<=0) return NET_ERRO;
...
int eva_login_verify_recv(EVA *eva,LONDATA* logindata){
    ...
    getucharfromevabyte(&encryptbyte,logindata->EVA_LOGIN_DD_TOKEN_key2,267,283);
    freeevabyte(&recvbyte),freeevabyte(&encryptbyte);
    return recvlen;
}
//0xE5
...
int eva_login_E5_recv(EVA *eva,LONDATA* logindata){
    ...
    getucharfromevabyte(&encryptbyte,logindata->EVA_LOGIN_E5_DATA02,274,397);
    freeevabyte(&recvbyte),freeevabyte(&encryptbyte);
    return recvlen;
}
//E3
...
int eva_login_E3_recv(EVA *eva,LONDATA* logindata){
    ...
    int recvlen=eva_net_recv(eva->net,&recvbyte);
    if(recvlen<=0) {printf("错误");return NET_ERRO;}
